...
function exec_service(){
  ...
  if [[ "${1}" == "follow" ]]; then
    docker exec -e MODE -e PORT -e ADDRESS=0.0.0.0 -it "${CONTAINER_ID}"  /test/systemd-rest_amd64
  else
    docker exec -e MODE -e PORT -e ADDRESS=0.0.0.0 -itd "${CONTAINER_ID}" /test/systemd-rest_amd64
  fi
}
